...
const ERRORES_MYSQL = {
  1054: [400, 'Se envio un campo que no existe en la tabla'],
  1048: [400, 'Falta un campo obligatorio'],
  1062: [409, 'Ya existe un registro con ese valor (cedula, codigo o email repetido)'],
  1264: [400, 'Un valor esta fuera del rango permitido'],
  1364: [400, 'Falta un campo obligatorio'],
  1366: [400, 'Un valor tiene un formato invalido'],
...
export function manejadorErrores(err, req, res, next) {
  ...
  if (ERRORES_CONEXION.includes(err.code)) {
    console.error(err);
    return res.status(503).json({ mensaje: 'No hay conexion con la base de datos' });
  }

  const conocido = ERRORES_MYSQL[err.errno];
  if (conocido) {
    const [estado, mensaje] = conocido;
    return res.status(estado).json({ mensaje: `${mensaje}: ${err.sqlMessage}` });
  }
